...
// the Move struct holds the source (tower being moved from) and dest (tower being moved to) as ints.
// the move number will be tracked simply by the position of a Move object in the moves queue
struct Move {
	int source;
	int dest;
};
...
int main() {
	...
	Queue<int> towers[NUM_TOWERS] = { Queue<int>(), Queue<int>(), Queue<int>() };
	...
	while (run) {
		...
		while (choose) {
			...
			try {
				comparison = towers[choice1].peek() < towers[choice2].peek();
			}
			...
			if (comparison) {
				try {
					towers[choice2].enqueue(towers[choice1].dequeue());
				}
				catch (QueueUnderflow e) {
		...
		if (isComplete(numDiscs, towers[NUM_TOWERS - 1])) {
			printTowers(towers);
...
void fill(int numDiscs, Queue<int>& tower) {
	for (int i = 0; i < numDiscs; i++) {
		tower.enqueue(numDiscs - i);
	}
}

// friend of Stack class
// prints the contents of the tower stack onto the screen
void displayTower(Queue<int>& tower) {
